/**
 * The residue tripwire (CPX-M2).
 *
 * Every V4 arc appended what it retired to `docs/architecture-v4.md` §14.1.
...
const SCAN_DIRS = ['docs', 'src', 'public', 'tests'];
const SCAN_FILES = ['README.md', 'wrangler.jsonc', 'app.py', '.gitignore'];
const TEXT = /\.(md|mjs|js|json|jsonc|html|css|py|yml|yaml|toml)$/;
...
/**
 * §13 (History) and §14.1 (Retired vocabulary) of the design ref are exempt:
 * history is allowed to say old words, and §14.1 IS the list. Computed from
 * the headings so the ranges cannot drift out of date.
 */
function designRefExemptRanges(lines) {
  const at = (p) => lines.findIndex((l) => l.startsWith(p)) + 1;
  return [
    [at('## 13.'), at('## 14.') - 1],
    [at('### 14.1'), at('## 15.') - 1],
  ];
...
test('no retired vocabulary in live prose or code (design ref §14.1)', () => {
  ...
  for (const file of files) {
    ...
    const exempt = rel === join('docs', 'architecture-v4.md')
      ? designRefExemptRanges(lines) : [];
 ...
 *    `worker-cache-contract.test.mjs` pins the values directly, which is a
