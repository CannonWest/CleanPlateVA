// The Worker's transport contract (CPF-M2 / CPH-M3), pinned under Vitest
// since CRC (2026-09-06; the node:test original ran untouched through
// every arc before it — the Worker did not change, the runner did). A direct
// import of src/worker.js replaces the original's data-URL import.
...
type R2Object = { body?: string | null; httpEtag: string }
type Env = {
    DATA_FULL: { get: (key: string, options?: { onlyIf?: Headers }) => Promise<R2Object | null> }
...
// acknowledgement (app/ack.ts), not this transport.
const env: Env = {
    DATA_FULL: {
...
const request = (path: string, init: RequestInit = {}) =>
    new Request(`https://cleanplateva.test/data-full/${path}`, init)
const noCache = () => { Reflect.deleteProperty(globalThis, 'caches') }
const ctx = () => {
    const pending: Promise<unknown>[] = []
    return { waitUntil: (p: Promise<unknown>) => { pending.push(p) }, pending }
}
...
function parseHeadersFile(text: string): [string, string[]][] {
...
test('the worker passes anything that is not /data-full/* to the assets layer untouched', async () => {
    ...
    // retired cache-control / re-404 logic on the way out. The sample paths
    // are the shell, a view path, the public data channel, and a hashed
    // build asset.
    const seen: string[] = []
    const passthrough: Env = {
        ...
        ASSETS: {
            fetch: async (req) => {
                ...
                return new Response('asset', { status: 200, headers: { 'Content-Type': 'application/json' } })
            },
        },
    }
...
function fakeCaches() {
    ...
    const calls = {
        match: [] as string[],
        put: [] as { url: string; cacheControl: string | null; status: number }[],
    }
...
test('a miss reads R2 and stores the copy; the next read is an edge HIT that never touches R2', async () => {
    ...
    const counting: Env = {
        ...env,
        DATA_FULL: { get: async () => { r2Reads++; return { body: '{"a":1}', httpEtag: '"e1"' } } },
    }
    const c = ctx()
...
test('a conditional request an unchanged object satisfies is a 304 with no body (R2 onlyIf on a miss)', async () => {
    noCache()
    let seenOnlyIf: Headers | undefined
    const conditional: Env = {
        ...env,
        DATA_FULL: {
...
test('the channel still refuses non-GET/HEAD, bad paths, and missing objects', async () => {
    ...
    const missing: Env = { ...env, DATA_FULL: { get: async () => null } }
    assert.equal((await worker.fetch(request('facility/NOPE.json'), missing, ctx())).status, 404)
})

test('the worker works without a Cache API (the test harness): plain MISSes, nothing thrown', async () => {
    noCache()
    const response = await worker.fetch(request('manifest.json'), env, ctx())
    assert.equal(response.status, 200)
    assert.equal(response.headers.get('X-Cache'), 'MISS')
