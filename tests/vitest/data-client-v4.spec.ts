/** The Contract V4 data client on the CR stack — ports
 *  `tests/data-client-v4.test.mjs` against `app/data/client.ts` (design ref
 *  §7 port order: FIRST — pins C1 whole). Same fixtures, same assertions;
 *  only the import target and the runner changed. The .mjs original keeps
 *  refereeing the served no-build client until CRC.
 */
import assert from 'node:assert/strict'
import { test } from 'vitest'
import {
    createFoodApi, decodeOverlay, DETAIL_CACHE_SIZE, type FetchLike, type ResponseLike,
} from '../../app/data/client'
import type { LoadedRoster, RosterResult, RosterUnavailable } from '../../app/data/types'
...
function fakeFetch(payloads: Payloads): FakeFetch {
    const calls: string[] = []
...
function loaded(result: RosterResult): LoadedRoster {
    assert.ok('facilities' in result, `expected a loaded roster, got ${JSON.stringify(result)}`)
    return result
}
...
const overlayShard = (bucket: string, finderSha: string, rows: unknown[]) => ({
    contract: 'cleanplateva.overlay-shard.v4', schema_version: 4, bucket,
    finder_sha256: finderSha, columns: COLUMNS, rows,
})
const closedShard = (bucket: string, facilities: unknown[]) => ({
...
test('the V4 full manifest loads finder + overlay in parallel and merges by position', async () => {
    ...
    // of the static public channel FIRST (CPH-M3, D-TRANSPORT-4); this
    // fixture has no static copy, so each shard falls back to R2.
...
test('the SPA shell on a data path is a miss, never markup to parse — full degrades to the basic map, the basic map reports unavailable', async () => {
    ...
    const none = await shellLite.getFoodFacilities() as RosterUnavailable
    ...
    const broken = await shellShard.getFoodFacilities() as RosterUnavailable
...
test('an overlay bound to a different finder shard is rejected — the client degrades to the finder', async () => {
    ...
    const result = loaded(await createFoodApi({ fetchImpl: fakeFetch(payloads) }).getFoodFacilities())
...
test('a misaligned overlay (row count) or a bad active count degrades to the finder', async () => {
    ...
    assert.equal(loaded(await createFoodApi({ fetchImpl: fakeFetch(misaligned) }).getFoodFacilities()).mode, 'lite')
    ...
    assert.equal(loaded(await createFoodApi({ fetchImpl: fakeFetch(wrongCount) }).getFoodFacilities()).mode, 'lite')
})
...
test('a V3 full manifest is not understood — no dual-contract path', async () => {
    ...
    const fetchImpl = fakeFetch(payloads)
    const result = loaded(await createFoodApi({ fetchImpl }).getFoodFacilities())
    assert.equal(result.mode, 'lite')
...
test('clicks read through one detail cache; the detail merges the roster row and decodes checklists', async () => {
    ...
    // all; the click path is the only detail reader.
    assert.equal('prefetchDetail' in api, false)  // retired-ok: asserts the retired prefetch is gone
    ...
    assert.equal((clicked.facility?.o as { grade_score?: number })?.grade_score, 91)
...
test('a detail with an unsupported contract is unavailable, not cached, and retried on the next click', async () => {
    ...
    // Not cached as a failure: the object was fetched again.
...
test('forced lite loads the V4 public finder and carries manifest freshness', async () => {
    ...
    const result = loaded(await createFoodApi({ fetchImpl, forceLite: true }).getFoodFacilities())
...
test('a public finder shard is not loaded without its V4 manifest, and V3 public data is refused', async () => {
    ...
    const missing = await createFoodApi({ fetchImpl: noManifest, forceLite: true }).getFoodFacilities() as RosterUnavailable
    ...
    const refused = await createFoodApi({ fetchImpl: v3, forceLite: true }).getFoodFacilities() as RosterUnavailable
