...
| **D-ACK-1** | When the acknowledgement appears | first load, map loading gray behind the modal, decline → Lite · on first judgment-bearing interaction · first load, blocking | first load (Cannon: the public *should* get Full) | CPF-M0/M1 | **resolved CPF-M0 2026-08-17 (Cannon) — first-load *blocking* modal (must answer; no dismiss-around) over the blank basemap; NO data fetch until the choice: Agree → Full, Reject → Lite (+ a persistent "Show inspection grades" control that re-opens it).** Not the recommendation: the recommended "Lite loads gray behind it" was rejected so no visitor pays Lite + Full on one visit (§9). Ships CPF-M1. |
| **D-ACK-2** | Attribution inventory + placement | modal only · modal + footer + About | modal + About; footer keeps the VDH line | CPF-M0 | **resolved CPF-M0 2026-08-17 — modal (short) + About §06 "Terms & attribution" (long form: the acknowledgement + per-upstream credit); footer keeps the VDH line and gains a "Terms & attribution" link.** Inventory decisions the same day: OSM = attribute + link ODbL only (the OSM-derived slice is ~3.6% of pins); Google = no visitor-facing mention (review-time discriminator; nothing Google-derived is published); Overture = "Overture Maps Foundation, overturemaps.org — CDLA-Permissive 2.0 and other open licences"; Foursquare OS = one line, © 2024 Foursquare Labs, Inc., Apache-2.0 link; our derived data = as-is, no warranty, credit CleanPlateVA, no licence grant. §06 landed **hidden** in the M0 PR; M1 unhides it with the modal + footer link. |
