...
| **CPD** — boot diet / Contract V4 | `cleanplate-boot-diet.md` | M0 measure + design ✅ **2026-08-16** (`cf_v4_prototype.py`; §7 measured — Lite 1.64 MB / Full 1.85 MB wire; D-DATA-1..9, -12, D-CUTOVER-3 resolved; retro in `cannon-food/references/retros/cpd-m0-v4-boot-diet.md`) · M1 cannon-food emits V4 ✅ **2026-08-16** (cground-skills #1667: `--contract v4` on exporter / publisher guard / incremental materializer with membership frozen / orchestrator; `test_export_contract_v4.py`; the shadowed-fallback fix; V4 materialized from Couch and dry-run against R2 — guard passed, delete 0) · M2 site V4 client ✅ built **2026-08-16**, **held as the cutover PR** (dataClient v4: finder+overlay position-merged and sha-checked, closed lazy, one detail LRU for hover + click; presentation/hover/list/about read the overlay + detail; V4 tripwires; **V4 Lite data + manifest committed in the branch**; QA'd on a throwaway port against the M1 alternate root incl. the V3-Full→V4-Lite degrade) · M3 cutover per §8 + V3 retirement · M4 stretch slimming (positional rows) if ever wanted | **ungated 2026-08-16** — CPR-M1a + M1b both merged; **M0 + M1 shipped, M2 built and held the same day** |
