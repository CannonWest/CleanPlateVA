...
| Sign-in affordance | "Sign in" button visible in Lite. | Replaced by the acknowledgement: a first-load blocking modal ("Agree and View Grades" / "Decline and Use Basic Map") and a persistent control on the map that re-opens the terms (label set at M1). |
...
| **CPF** — public Full | `cleanplate-public-full.md` | **M0 ✅ 2026-08-17** — attribution inventory (10 upstreams, verified against `cannon_food/location/*` + upstream policy pages; provenance of the 27,919 published pins: VGIN 92.5% · OSM/Nominatim 3.6% · Census 2.2% · ZIP-centroid 0.8% · Overture 0.6% · Foursquare 0.3% · manual 0.1%), plan tier (Free), the Access app inspected (`CleanPlateVA full data`, `c9a0d19f-4935-4344-b1e5-bd7a0a16d30e`, apex-scoped, one email policy), D-ACK-1..3 + D-TRANSPORT-1 resolved, the copy approved by Cannon and the About §06 "Terms & attribution" section landed **hidden** (`d-none`, M1 unhides) · M1 ack UX (blocking first-load modal over the blank basemap, `localStorage`, accept → Full / decline → Lite, replaces sign-in button, About/footer wording, unhide §06; embed acks once per origin — no CannonAI change) · M2 transport per §9 (public headers + Cache API) · M3 Access app retirement + verification + README/memory · M4 observe requests/day + hit rates; consider D-TRANSPORT-2 | **shipped gate** — CPD-M3 live; M0 done, M1 next |
